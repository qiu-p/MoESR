...
def convertImage(mask_img, origin_img, n_e, n_d):
    ...
    return out_img, mask2_e_img, mask2_ed_img, mask2_ede_img

@IMG_PROCESS_REGISTRY.register()
def mask(path, opt, origin_img=None):
    ...
    mask_ed_save_name = mask_basename + '_ed' + ext
    mask_ed_path = osp.join(mask_save_folder, mask_ed_save_name)
    mask_ede_save_name = mask_basename + '_ede' + ext
    mask_ede_path = osp.join(mask_save_folder, mask_ede_save_name)
    ...
    # convert
    out_img, _, mask_ed_img, mask_ede_img = convertImage(mask_img, origin_img, n_e=structure_element_kernel_e_size, n_d=structure_element_kernel_d_size)

    # save
    out_img.save(out_path)
    mask_ed_img.save(mask_ed_path)
    mask_ede_img.save(mask_ede_path)
